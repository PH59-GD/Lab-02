...
def AddEm():
    ...
    GP = BP + EA
    NP = GP - ED - ET
    myEmployees.update({"Employee"+str(len(myEmployees)+1): {"Employee Name": EN, "Basic Pay": BP, "Employee Allowance": EA, "Employee Deductions": ED, "Employee Taxes": ET, "Gross Pay": GP, "Net Pay": NP}})
def DelEm():
...
def DisplayEm():
    if (len(myEmployees) > 0):
        ...
        for i in range(len(myEmployees)):
            i += 1
            print("Employee"+str(i)+": ")
            print(myEmployees["Employee"+str(i)])
    else:
        print("No Employees!")
while True:
    print("")
